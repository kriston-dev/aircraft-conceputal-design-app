...
function user_path = user_state()
    ...
    user_path = input("My choice is: ");
    
    if user_path == "Find T/W"
        currState = State.FIND_TW;
    end
end
...
q_accel = 0.5 .* rho_accel .* velocity_accel.^2;
...
Thrust_to_Weight_horiz_accel = (q_accel .* C_D_accel) ./ Wing_loading ...
    + accel_horiz ./ Gravity; % Horizontal acceleration
...
%variables
    ...
    %User input
rho_approach = 1.225;
velocity_approach = 70 .* knot_to_ms;
K_approach = 1.3;
%sub-formulas

velocity_stall_approach = velocity_approach ./ K_approach;
q_approach = 0.5 .* rho_approach .* velocity_stall_approach.^2;

%approach formula constraint
Wing_loading_approach = q_approach .* C_Lmax_TO;
