...
while true
    switch currState
        ...
        case Aircraft_constraint_states.SAVED_CUSTOM_AIRCRAFT_DATA
            doc_options();
...
Wing_skin_area_total = 2 .* user_aircraft_input.Wing_area_raw;
...
Wing_loading = Weight_TO ./ user_aircraft_input.Wing_area_raw;
AR_wing = user_aircraft_input.Engine_power_HP.^2 ./ user_aircraft_input.Wing_area_raw;
...
user_aircraft_input.rho_TO = ...
    ...
    %Velocity formulas
Velocity_stall = sqrt((2 .* Weight_TO) ./ (user_aircraft_input.rho_TO .* user_aircraft_input.Wing_area_raw .* user_aircraft_input.C_Lmax_TO));
...
C_L_required_TO = 2 .* Weight_TO ./(rho_SL .* Velocity_TO.^2 .* user_aircraft_input.Wing_area_raw);
...
Lift_TO = 0.5 .* rho_SL .* Velocity_TO.^2 .* user_aircraft_input.Wing_area_raw .* C_L_required_TO;

Lift_avg_TO = 0.5 .* rho_SL .* Velocity_avg_TO.^2 .* user_aircraft_input.Wing_area_raw .* C_L_required_TO;
...
Drag_TO = 0.5 .* C_DTO .* rho_SL .* user_aircraft_input.Wing_area_raw .* Velocity_TO.^2;

Drag_avg_TO = 0.5 .* rho_SL .* Velocity_avg_TO.^2 .* user_aircraft_input.Wing_area_raw .* C_DTO;
...
Thrust_to_Weight_climb = Beta/alpha .* (((K_1 .* n.^2 .* Beta) ./ ...
    q_climb) .* (Weight_TO ./ user_aircraft_input.Wing_area_raw) + K_2 .* n + C_D0 ./ ...
...
q_cruise = 0.5 .* user_aircraft_input.rho_cruise .* velocity_cruise_knts.^2;
...
velocity_stall_straight = sqrt((2 .* Wing_loading) ...
    ./ (user_aircraft_input.rho_turn .* user_aircraft_input.C_Lmax_TO)); %A straight line of aircraft stall velocity
...
for refine_loop = 1:1000
    ...
    velocity_stall_turn = sqrt((2 * Wing_loading .* n) ...
        ./ (user_aircraft_input.rho_turn .* user_aircraft_input.C_Lmax_TO));
...
velocity_stall_approach = velocity_approach ./ user_aircraft_input.K_approach;
q_approach = 0.5 .* user_aircraft_input.rho_approach .* velocity_stall_approach.^2;

%approach formula constraint
Wing_loading_approach = q_approach .* user_aircraft_input.C_Lmax_TO;
...
user_aircraft_input.velocity_climb = input("Climb velocity (m/s): ");
...
user_aircraft_input.velocity_cruise = input("Cruise velocity knots: ");
...
user_aircraft_input.velocity_approach = input("Approach velocity (m/s): ");
