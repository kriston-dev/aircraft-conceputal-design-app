...
clearvars -except user_aircraft_TW_input
clear functions
clear classes
...
while true
    switch currState
        ...
        case CALCULATE_ST
            ST = calculate_ST_constraints(user_ST_aircraft_input, ...
                user_ST_aircraft_input);
            disp(ST.Tail_moment_arm);
...
user_aircraft_TW_input.C_Lmax_turn = input("Turn max lift coefficent: ");
...
disp("5  - Find the Stability/Trim");
...
user_ST_aircraft_input.Wing_area = input("Choose this number that are above" + ...
    " the constraints from the find T/W selection");

user_ST_aircraft_input.Horizontal_tail_volume_coeff = ...
    input("Choose this number by aircraft type: ");

user_ST_aircraft_input.Vertical_tail_volume_coeff = ...
    input("Choose this number by aircraft type: ");

user_ST_aircraft_input.Cockpit_radius = input("Choose this number based on" + ...
    " your idea of the preference you want the cockpit: ");

user_ST_aircraft_input.Fuselage_radius_near_tail = input("What is the " + ...
...
function ST = calculate_ST_constraints(user_aircraft_TW_input, ...
    user_ST_aircraft_input)
...
MAC_ref = user_aircraft_TW_input.Wing_area ./ user_aircraft_TW_input.span;
...
ST.Tail_moment_arm = sqrt((2 .* user_ST_aircraft_input.Wing_area .* ...
    ...
    (pi .* (user_ST_aircraft_input.Cockpit_radius + Fuselage_radius_near_tail)));
...
%Goals for impropvement for accuracy in the future:
    ...
    %Based on a rectangular wing, i would like to add an option for the
%user to add their own Mac_ref if they would want to and the option to
%calulate a swept wing with any type of curvature from the wing tips in the
%get stability/trim

%have the Weight change each moment for calculation due to the weight from
%fuel constantly changing in each phase of the find T/W