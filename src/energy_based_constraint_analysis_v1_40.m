...
if isfile("user_loads_aircraft.mat")
    ...
    user_loads_saved = struct('name', {}, 'loads_aircraft_name', {}, 'user_input', {});
...
%This calculates what the user entered from getting the S/T aircraft data
        case Aircraft_constraint_states.CALCULATE_ST
            ...
            currState = Aircraft_constraint_states.GRAPH_ST;



%This graphs what was calculated from getting the S/T aircraft data
        case Aircraft_constraint_states.GRAPH_ST
...
%This calculates the specific S/T aircraft the user chose
        case Aircraft_constraint_states.CALCULATE_ST_FROM_USER_ST_DATA_CALCULATION
            ...
            currState = ...
                Aircraft_constraint_states.GRAPH_ST_FROM_USER_ST_DATA_CALCULATION;



%This graph the specific S/T aircraft the user chose
        case Aircraft_constraint_states.GRAPH_ST_FROM_USER_ST_DATA_CALCULATION
            display_ST_calc_data(ST);
...
%This calculates what the user entered from getting the S/T aircraft data
        case Aircraft_constraint_states.CALCULATE_LOADS
            loads = calculate_ST(user_aircraft_TW_input, ...
                user_loads_aircraft_input);

            currState = Aircraft_constraint_states.GRAPH_LOADS;



%This graphs what was calculated from getting the S/T aircraft data
        case Aircraft_constraint_states.GRAPH_LOADS
            display_ST_calc_data(loads);
...
%This calculates the specific S/T aircraft the user chose
        case Aircraft_constraint_states.CALCULATE_LOADS_FROM_USER_LOADS_DATA_CALCULATION
            loads = calculate_loads(user_ST_aircraft_input, ...
                user_loads_aircraft_input);

            currState = ...
                Aircraft_constraint_states.GRAPH_LOADS_FROM_USER_LOADS_DATA_CALCULATION;



%This graph the specific S/T aircraft the user chose
        case Aircraft_constraint_states.GRAPH_LOADS_FROM_USER_LOADS_DATA_CALCULATION
...
Wing_loading = Weight_TO ./ user_aircraft_TW_input.Wing_area;
...
C_L_required_TO = 2 .* Weight_TO ./(user_aircraft_TW_input.rho_TO .*...
...
% /*Lift_TO = 0.5 .* user_aircraft_TW_input.rho_TO .* Velocity_TO.^2 .*
...
Thrust_to_Weight_climb = Beta/alpha .* (((K_1 .* n.^2 .* Beta) ./ ...
    q_climb) .* (Weight_TO ./ user_aircraft_TW_input.Wing_area) + K_2 .* ...
...
user_ST_aircraft_input.TR = input("Based on past aircraft choose a taper ratio " + ...
    "that resembles your aircraft: ");

user_ST_aircraft_input.tail_efficiency = input("Horizontal tail efficiency factor (0 to 1): ");
...
MAC_ref = (2 ./ 3) .* ST.root_chord .* ((1 + user_ST_aircraft_input.TR + ...
...
ST.Tail_moment_arm = sqrt((2 .* user_ST_aircraft_input.Wing_area .* ...
    ((user_ST_aircraft_input.Horizontal_tail_volume_coeff .* MAC_ref) + ...
     (user_ST_aircraft_input.Vertical_tail_volume_coeff .* ...
...
ST.HS_Wing_area = (user_ST_aircraft_input.Horizontal_tail_volume_coeff .* ...
    user_ST_aircraft_input.Wing_area .* MAC_ref) ./ ST.Tail_moment_arm;
...
wing_AR_ref = user_aircraft_TW_input.Span.^2 ./ ...
...
wing_lift_curve_slope = (2 .* pi .* wing_AR_ref) ./ ...
    (2 + sqrt(4 + wing_AR_ref.^2));

Y_MAC = (user_aircraft_TW_input.Span ./ 6) .* ((1 + 2 .*...
    user_ST_aircraft_input.TR) ./ (1 + user_ST_aircraft_input.TR));

X_root_LE = 0; %The wing root LE is our x-datum

X_leading_edge_MAC = X_root_LE + Y_MAC .* tand(user_aircraft_TW_input.Sweep_angle); 

X_AC = X_leading_edge_MAC + 0.25 .* MAC_ref;

downwash_gradient = (2 .* wing_lift_curve_slope) ./ (pi .* wing_AR_ref);

    %location of neutral point based on entire aircraft
ST.X_NP = X_AC + user_ST_aircraft_input.tail_efficiency .* ...
    user_ST_aircraft_input.Horizontal_tail_volume_coeff .* ...
    (tail_lift_curve_slope ./ wing_lift_curve_slope) .* ...
    (1 - downwash_gradient) .* MAC_ref;
...
disp("Neutral location lcoated at: " + ST.X_NP);
...
user_loads_aircraft_input.Wing_area = input("The wing area that" + ...
    " you chose, enter in that specific wing area that founded from the wing" + ...
    " loading: ");
...
function loads = calculate_loads(user_loads_aircraft_input, ...
    user_ST_aircraft_input)


loads.weight = user_loads_aircraft_input.mass .* 1 .* user_ST_aircraft_input.Wing_area;

%Static margin
% ST.static_margin = (ST.X_NP - ST.X_CG) ./ MAC_ref;
...
elseif strcmpi(user_save_option, "no")
    ...
    disp("input was not yes or no");
    user_loads_saved = save_user_ST_aircraft(user_loads_saved, ...
    user_loads_aircraft_input, ST_aircraft_name);
...
switch user_choice
    ...
    case 1
        user_loads_aircraft_input.Wing_area = ...
            input("Weight: ");
