...
user_loads_aircraft_input.alpha_L0_wing = deg2rad(input("Enter the wing zero-lift angle of attack from" + ...
    " DATCOM (deg): "));
...
loads.AFT_X_CG = ST.X_NP - loads.static_margin .* ST.MAC_ref;
...
%and elevator deflecitno rather htan asking the user because this can cause
...
alpha_TO = user_loads_aircraft_input.alpha_L0_wing + TW.C_L_required_TO ./ ...
...
downwash_TO = downwash_gradient .* (alpha_TO - user_loads_aircraft_input.alpha_L0_wing);

%The AoA the horizontal stabilizer is experiencing
AoA_HS = alpha_TO + user_loads_aircraft_input.tail_incidence - downwash_TO;

delta_e_forward_CG = -deg2rad(user_loads_aircraft_input.max_elevator_up);

C_L_tail_forward = tail_lift_curve_slope .* ...
    (AoA_HS - alpha_L0_tail + ...
