...
%I need to fix:
%For some reason it opens a grpah after runnnig the calculation of aircraft
%loads.

%We need to move the XCG to 2.9 instead of 3.2 meters.

%We need to create the edit case to change values XCG after we officially
%create the user aircraft loads data
...
wing_ref_lift_required_TO = TW.q_TO .* TW.C_L_required_TO .* user_ST_aircraft_input.Wing_area;
...
%changes
wing_lift_curve_slope = (2 .* pi .* ST.wing_AR_ref) ./ (2 + sqrt(4 + ST.wing_AR_ref.^2));

% C_L_ref_wing = (TW.Weight_TO - lift_tail) ./ (ST.q_TO .* ST.wing_AR_ref);

%The AoA that is required to takeoff
AoA_TO = user_loads_aircraft_input.alpha_L0_wing + TW.C_L_required_TO ./ ...
...
%Longitudinal trim
% loads.elevator_deflection_trimmed = -(C_M0 + C_M_AoA .* AoA) ./ elevator_control_efficiency;
...
disp("31 - Wing pitching moment coefficient C_M_AC_TO");
disp("32 - Horizontal tail lift coefficient C_L_tail_TO");
...
switch user_choice
    ...
    case 31
        user_loads_aircraft_input.C_M_AC_TO = ...
            input("Reference wing pitching moment coefficient during takeoff: ");

    case 32
        user_loads_aircraft_input.C_L_tail_TO = ...
            input("Horizontal tail lift coefficient during takeoff: ");
...
%tail region radius
%fuleslage radii between the cockpit and tail radii to create body shape
%Horizontal tial taper ratio
%Horizontal tale LE sweep
%Horizontal Tail incidence
%horizontal tail root leading edge



%Fix the TO lift claculation so the wing lift takes in account for the
%horizontal stabilzier force. When hte tial produce downforce, the wing
%needs to produce the added lift so the total lift of the aircraft still
%equals the weight. we can solve for the lift coefficcient from the wing by
%the current weight and subtract from the lift of the tial and then we
%divide all of that of the dynamic pressure being multiplied with the
%reference wing area. so 