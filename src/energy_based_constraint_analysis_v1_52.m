...
user_loads_aircraft_input.nose_weight_factor = input("What decimal " + ...
...
loads.limit_load_factor = n_limit .* TW.Weight_TO;

%ultimate load

loads.ult_load_factor = 1.5 .* limit_load_factor;

%Wing bending moment

loads.moment_ref_wing_root = ( n .* (TW.weight_TO - ...
    user_aircraft_TW_input.Span)) ./ 8;

%Gust loads, vertical gust changes load factor

loads.delta_load_factor = (rho_cruise .* TW.velocity_cruise .* tail_lift_curve_slope) ... 
    ./ (2 .* (TW.Weight_TO ./ user_ST_aircraft_input.Wing_area));

loads.total_load_factor = 1 + loads.delta_load_factor;


% Landing gear vertical loads

loads.landing_gear_vert_load = n .* ...
    (user_loads_aircraft_input.Landing_nose_gear_mass + ...
    user_loads_aircraft_input.Landing_main_gear_mass);

%Main gear loads if main wheel carries 90% of weight...

loads.Force_main_landing_gear_both = user_loads_aircraft_input.main_weight_factor .* n.* TW.Weight_TW;
    %Then each main wheel is this value but divided by 2

%Nose force based off the user inputs from the main landing gear

user_loads_aircraft_input.nose_weight_factor = 1 - ...
    user_loads_aircraft_input.main_weight_factor;

    loads.Force_nose_landing_gear = user_loads_aircraft_input.main_weight_factor ...
        .* n .* TW.Weight_TO;

%Performance verification
    %will be updated in version 2...
