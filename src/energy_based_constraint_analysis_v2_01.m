...
%Note: I ran the program and realized there are grammar errors and wrong
%sentences for number 44 when asking the user the load factor in gs.
...
%This graphs what the calculation gave from getting the T/W aircraft data
        case Aircraft_constraint_states.GRAPH_TW
            ...
            %Display the structure of all the variables saved to TW
        case Aircraft_constraint_states.DISPLAY_TW_FROM_USER_ST_AIRCRAFT_MENU

            disp(" ");
            disp("This was the output of the desired Wing area you chose " + ...
                "from the energy based constraint analysis");
            disp(" ");
...
if user_aircraft_TW_input.Sweep_angle == 0
    TW.e = 1.78 .* (1 - 0.045 .* AR_wing.^0.68) - 0.64;
elseif user_aircraft_TW_input.Sweep_angle >= 30
    TW.e = 4.61 .* (1 - 0.045 .* AR_wing.^0.68) .* ...
...
elseif (user_aircraft_TW_input.Sweep_angle > 0) && ...
    ...
    TW.e0 = 1.78 .* (1 - 0.045 .* AR_wing.^0.68) - 0.64;
    TW.e30 = 4.61 .* (1 - 0.045 .* AR_wing.^0.68) .* (cosd(30)).^0.15 - 3.1;
    TW.e = TW.e0 + (user_aircraft_TW_input.Sweep_angle ./ 30) .* (TW.e30 - TW.e0);
else
    error("Sweep angle is out of range")
end
    
    %Full Drag Polar Buildup
TW.K_1 = 1./(pi .* AR_wing .* TW.e);

TW.C_D0 = user_aircraft_TW_input.C_Dmin + TW.K_1.*user_aircraft_TW_input.C_Lmin.^2;

TW.K_2 = -2.*TW.K_1.*user_aircraft_TW_input.C_Lmin;
...
user_aircraft_TW_input.rho_TO = ...
    ...
    %Velocity formulas
TW.Velocity_stall = sqrt((2 .* TW.Weight_TO) ./ (user_aircraft_TW_input.rho_TO .*...
    user_aircraft_TW_input.Wing_area .* user_aircraft_TW_input.C_Lmax_TO));

TW.velocity_TO = user_aircraft_TW_input.K_TO * TW.Velocity_stall;

TW.Velocity_avg_TO = TW.velocity_TO ./ sqrt(2);

    %constraints sub-formulas
TW.q_avg_TO = 0.5 .* user_aircraft_TW_input.rho_TO .* TW.Velocity_avg_TO.^2;

C_L_required_TO = 2 .* TW.Weight_TO ./(user_aircraft_TW_input.rho_TO .*...
    TW.velocity_TO.^2 .* user_aircraft_TW_input.Wing_area);
...
TW.C_DTO = TW.C_D0 + (TW.K_1.*C_L_required_TO.^2) + TW.K_2.*C_L_required_TO;
...
Acceleration_TO = TW.velocity_TO.^2 ./ (2 .* user_aircraft_TW_input.S_G);

Thrust_to_Weight_TO = (Acceleration_TO ./ Gravity) + ... 
    (TW.q_avg_TO .* TW.C_DTO) ./ Wing_loading + user_aircraft_TW_input.Rolling_friction_coefficient ...
    .* (1 - (TW.q_avg_TO .* C_L_required_TO) ./ Wing_loading); % Takeoff Constraint
...
Thrust_to_Weight_climb = Beta/alpha .* (((TW.K_1 .* n.^2 .* Beta) ./ ...
    q_climb) .* (TW.Weight_TO ./ user_aircraft_TW_input.Wing_area) + TW.K_2 .* ...
    n + TW.C_D0 ./ ((Beta ./ q_climb) .* Wing_loading) + ...
...
C_D_cruise = TW.C_D0 + (TW.K_1 .* C_Lcruise.^2) + TW.K_2 .* C_Lcruise;
...
C_D_turn = TW.C_D0 + TW.K_1 .* C_L_turn.^2 + TW.K_2 .* C_L_turn;
...
C_D_accel = TW.C_D0 + TW.K_1 .* C_L_accel.^2 + TW.K_2 .* C_L_accel;
...
TW.q_TO = 0.5 .* user_aircraft_TW_input.rho_TO .* TW.velocity_TO.^2;
...
ST.tail_lift_curve_slope = (2 .* pi .* user_ST_aircraft_input.AR_HS) ./ ...
    (2 + sqrt(4 + user_ST_aircraft_input.AR_HS.^2));

ST.wing_lift_curve_slope = (2 .* pi .* ST.wing_AR_ref) ./ ...
    (2 + sqrt(4 + ST.wing_AR_ref.^2));

ST.Y_MAC = (user_aircraft_TW_input.Span ./ 6) .* ((1 + 2 .*...
    user_ST_aircraft_input.TR) ./ (1 + user_ST_aircraft_input.TR));

ST.X_leading_edge_MAC = user_ST_aircraft_input.X_wing_root_LE + ST.Y_MAC .* tand(user_aircraft_TW_input.Sweep_angle); 

%The aerodynamic center location on a 1D plane (the longitude, left to right)
ST.X_AC = ST.X_leading_edge_MAC + 0.25 .* ST.MAC_ref;

ST.downwash_gradient = (2 .* ST.wing_lift_curve_slope) ./ (pi .* ST.wing_AR_ref);

    %location of neutral point based on entire aircraft
ST.X_NP = ST.X_AC + user_ST_aircraft_input.tail_efficiency .* ...
    user_ST_aircraft_input.Horizontal_tail_volume_coeff .* ...
    (ST.tail_lift_curve_slope ./ ST.wing_lift_curve_slope) .* ...
    (1 - ST.downwash_gradient) .* ST.MAC_ref;
...
disp("6  - From this WING AREA you chose in this saved file, find the " + ...
    "information that was collected from the energy based constraint analysis");
...
switch user_choice
    ...
    case 6
        currState = ...
            Aircraft_constraint_states.DISPLAY_TW_FROM_USER_ST_AIRCRAFT_MENU;
...
user_loads_aircraft_input.C_M0 = input("Full aircraft pitching moment coeff " + ...
...
user_loads_aircraft_input.limit_load_factor = input("Enter the aircraft " + ...
    "structural limit load factor (g): ");
...
TW.total_mass = sum(initial_masses);

% The longitudinal CG datum location at the aircraft nose (1D)
loads.X_CG = sum(initial_masses .* X_components) ./ TW.total_mass;
...
loads.X_tail_AC = ST.X_AC + ST.Tail_moment_arm;
...
for updated_tail_force = 1:100
    ...
    C_L_tail_forward = ST.tail_lift_curve_slope .* ...
        (AoA_HS - AoA_L0_HS + ...
...
loads.forward_X_CG = (ST.X_AC .* wing_ref_lift_required_TO + ...
   tail_force .* loads.X_tail_AC - loads.wing_pitching_moment) ./ ...
...
loads.Force_nose_landing_gear = loads.nose_gear_reaction_factor .* TW.Weight_TO;
...
%Throughout the process of users inpuytting their data, we need to make
%each input statement a case and save the info to the user file when moving
%to the next case. Thus if they exit the program in the middle, it saves
%their position in that chain of input statements and continues them where
%they last were.
